# -*- coding: utf-8 -*-
"""
增加、删除、修改YAML配置项 v0.0.1改
沙盒移植版本，有较大的修改，该版本仅为沙盒环境，不推荐正常使用
"""
from .. import lib

class ManagerError(Exception):
        def __init__(self, message="",parent_error:Exception=None):
            self.message = message
            self.parent_error=parent_error
            super().__init__(self.message)
        def __str__(self):
            if self.parent_error is None:
                return f"{self.message}"
            elif isinstance(self.parent_error,Exception):
...
class YamlFileManager:
    ...
    def _read_yaml(self):
        ...
        try:
            ...
            raise ManagerError(f"An error occurred while reading the file: {self.file_path}",parent_error=e)
    def _write_yaml(self,data:dict=None):
        ...
        try:
            ...
            raise ManagerError(f"An error occurred while writing to the file: {self.file_path}",parent_error=e)
    def get_data(self):
        """获取 YAML 数据"""
